...
module dense_layer #(
    ...
    localparam [2:0] S_IDLE=0, S_START=1, S_ADDR=2, S_PIPE=3,
                     S_FEED=4, S_WAIT=5, S_DONE=6;
    reg [2:0] st;
    ...
    // --- FSM: 3 cycles per input (ADDR → PIPE → FEED) ---
    //     matches neuron's 3-cycle startup (S_IDLE → S_B1 → S_B2 → S_MAC)
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            S_ADDR:  st<=S_PIPE;                   // ← NEW: extra BRAM latency
            S_PIPE:  st<=S_FEED;                   // ← NEW
            S_FEED: begin
                n_din       <= in_data;
                n_din_valid <= 1'b1;
                if (cnt == NI - 1)
