...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (st)
        S_SYNC: begin cnt<=0;
                    if (rx_valid && rx_data==8'hAA) st<=S_HI; end
        ...
        S_WAIT: if (cnn_result_valid)
                    begin res_r<=cnn_digit; st<=S_TX; end
        S_TX:   if (!tx_busy)
                    begin tx_data<={4'h0,res_r}; tx_send<=1; st<=S_TXW; end
