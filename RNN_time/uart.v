    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_DATA:
                    if (clk_cnt == CLKS_PER_BIT[15:0] - 1'd1) begin
                        clk_cnt        <= 0;
                        shift[bit_idx] <= rx_s2;
...
module uart_tx #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state <= S_IDLE; tx <= 1'b1; busy <= 1'b0;
            clk_cnt <= 0; bit_idx <= 0; shift <= 0;
        end else begin
            case (state)
                S_IDLE: begin
                    tx <= 1'b1; busy <= 1'b0;
                    if (send) begin
                        shift <= data; busy <= 1'b1;
                        clk_cnt <= 0;  state <= S_START;
                    end
                end
                S_START: begin
                    tx <= 1'b0;
